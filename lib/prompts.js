...
- **Sampled**: all free text — captions, comments, messages, prompts, video titles. \`coverage.sampling\` says how many of how many. Messages in particular are a deliberately small, structured draw: half from the last eighteen months and half from before that, and within each half, half their longest and half drawn at random. So the sample is balanced across eras by construction — it does *not* tell you that they wrote as much then as now, and the counts are where you read volume.
